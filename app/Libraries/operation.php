...
if (empty($_REQUEST[$keyName]) && $t_module["param"] == "display"){
    ...
    $origine == "document" ? $this->id = $_REQUEST[$keyName] : $this->id = $_SESSION["ti_operation"]->getValue($_REQUEST[$keyName]);
}
...
try {
        ...
        if ($this->id > 0) {
            $_REQUEST[$keyName] = $_SESSION["ti_operation"]->setValue($this->id);
        }
