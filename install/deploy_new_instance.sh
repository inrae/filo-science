...
REPO=https://github.com/inrae/filo-science
PHPVER=8.3
PHPINIFILE="/etc/php/$PHPVER/apache2/php.ini"
echo "Installation of filo-Science app "
...
git clone $REPO -b main

# update rights on files
chmod -R 755 filo-science/
cd filo-science
# create .env file
cp env .env
chgrp www-data .env
...
openssl genpkey -algorithm rsa -out id_filo -pkeyopt rsa_keygen_bits:2048
openssl rsa -in id_filo -pubout -out id_filo.pub
chown www-data id_filo
...
cp install/apache2/filo2.conf /etc/apache2/sites-available/
/usr/sbin/a2ensite filo2
echo "you must modify the file /etc/apache2/sites-available/filo2.conf"
